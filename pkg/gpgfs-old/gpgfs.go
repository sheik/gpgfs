...
func (r *GPGRoot) Rename(ctx context.Context, name string, newParent fs.InodeEmbedder, newName string, flags uint32) syscall.Errno {
	...
	gpgfile := &GPGFile{root: r, file: r.GetPath() + "/" + newName, name: newName}
	ch := r.Inode.NewPersistentInode(ctx, gpgfile, fs.StableAttr{})
	r.Inode.RmChild(name)
	r.Inode.RmChild(newName)
	r.Inode.AddChild(newName, ch, true)

	/*
		p := newParent.(*GPGRoot)

		//	ch := r.EmbeddedInode().GetChild(name).EmbeddedInode()
		gpgfile := &GPGFile{
			Inode: fs.Inode{},
			file:  newName,
			name:  newName,
...
func (file *GPGFile) Getattr(ctx context.Context, f fs.FileHandle, out *fuse.AttrOut) syscall.Errno {
	path := file.root.GetPath() + "/" + file.name
	if file.link != "" {
...
func (file *GPGFile) Open(ctx context.Context, flags uint32) (fs.FileHandle, uint32, syscall.Errno) {
	...
	fmt.Println("setting data to:", decrypted)
